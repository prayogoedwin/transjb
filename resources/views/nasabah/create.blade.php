<x-layouts.app>
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Tambah Nasabah') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Tambahkan nasabah baru') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden max-w-xl">
        <div class="p-6">
            <form action="{{ route('nasabah.store') }}" method="POST" class="max-w-2xl">
                ...
                <div class="mb-4">
                    <x-forms.input label="Nama" name="nama" type="text" value="{{ old('nama') }}" required />
                </div>

                <div class="mb-4">
                    <x-forms.input label="No. Telepon" name="no_telp" type="text" value="{{ old('no_telp') }}" />
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="alamat" rows="4" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-100">{{ old('alamat') }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
